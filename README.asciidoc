...
| deno a| Process the response with JavaScript running in a deno instance. The fields `status`, `headers` and `body` allow you to inspect the response. Call the `setResult()` function with the result of your processing script. If you don't call `setResult` in your script, the result will be equal to `body`. For debugging purposes there are the `print` and `printerr` functions.
